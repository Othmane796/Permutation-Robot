...
void jeu(int nb_cases,int nb_couleurs,int choix){
  ...
  while(continuer)
  {
  	 ...
  	 if(click && c_click->p !=NULL){
   		if(c_click->p->couleur != c_click->couleur){
  			...
  			if(bougerPiece_bis(tab[i],c_click)){
  				...
  				click=0;
  				indice_pile--;
  				afficher_text(ecran,"Can't move this piece",black,position_mot,35);
  				SDL_Flip(ecran);
  	...
  	if(indice_pile==nb_pieces){
  		...
  		afficher_text(ecran,changements,black,position_cmpt,50);//ma fonction afficher text prend un char  va dans text.c
  		SDL_Flip(ecran);
  		afficher_text(ecran,"Finished : To go back to the menu press \" ESC \" or \" X \"",black,position_close,25);
  		SDL_Flip(ecran);
  	...
  	*/
  	...
  	SDL_Flip(ecran);
  		...
  		printf("Blocked : There is no solution\n");
